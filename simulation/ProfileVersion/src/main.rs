...
fn main() -> io::Result<()>{
    ...
    let result: Vec<io::Result<()>> = files.iter().map(|file| {
        if let Some(path) = file.path().to_str() {
            let path_split: Vec<_> = path.split("/").collect();
            let profile_path = "/Volumes/KINGSTON/Coursework/modern_limits_of_ilp_18742/sim_results/exp7/".to_string() + path_split[6] + "/" + path_split[7] + "/profile.csv";
            let sim_path = "/Volumes/KINGSTON/Coursework/modern_limits_of_ilp_18742/sim_results/exp7/".to_string() + path_split[6] + "/" + path_split[7] + "/sim.csv";
