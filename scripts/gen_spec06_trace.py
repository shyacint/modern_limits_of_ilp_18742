...
def run_qemu_trace(benchmark, output_dir, compress=False, timeout=None):
    ...
    for binary, args, trace_name, out_name in SPEC_BENCHMARK_TO_QEMU_COMMAND_MAP[benchmark]:
        ...
        args_full = [os.path.join(exe_dir, arg) if os.path.exists(os.path.join(exe_dir, arg)) else arg for arg in args]

        trace_path = os.path.join(benchmark_outdir, trace_name)
        out_path = os.path.join(benchmark_outdir, out_name)

        qemu_cmd = [QEMU_EXECUTABLE, "-d", "in_asm,exec,nochain", binary_path] + args_full
        ...
        try:
            with open(trace_path, "w") as trace_log, open(out_path, "w") as stdout_log:
                subprocess.run(qemu_cmd, stdout=stdout_log, stderr=trace_log, timeout=timeout)
            elapsed = time.time() - start_time
            print(f"[INFO] Benchmark completed in {elapsed:.2f} seconds")
        except subprocess.TimeoutExpired:
            print(f"[WARN] Benchmark timed out after {timeout / 60:.2f} minutes")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--timeout", type=int, default=5, help="Timeout in minutes for each benchmark (default: 5)")

    args = parser.parse_args()
    timeout_seconds = args.timeout * 60
    if args.all:
        for benchmark in SPEC_BENCHMARK_TO_QEMU_COMMAND_MAP.keys():
            run_qemu_trace(benchmark, args.outdir, args.compress, timeout=timeout_seconds)
    elif args.benchmark:
        run_qemu_trace(args.benchmark, args.outdir, args.compress, timeout=timeout_seconds)
    else:
